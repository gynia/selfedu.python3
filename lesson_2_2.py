...
# pr = property(get_pr, set_pr, del_pr) - Этот вариант правильно 
# определяет свойство с геттером, сеттером и делитером. 
# Такой подход позволяет контролировать доступ к атрибуту и его 
# изменение, а также удаление.

# pr = property(get_pr, set_pr) - В этом варианте у нас есть только 
# геттер и сеттер, но нет делитера. Этот вариант также допустим, если 
# вам не нужно поддерживать операцию удаления атрибута.

# pr = property(get_pr) - В этом варианте у нас есть только геттер, но 
# нет сеттера и делитера. Это также допустимо, если вы хотите только 
# читать значение атрибута, но не изменять или удалять его.

# pr = property(); pr = pr.setter(set_pr); pr = pr.getter(get_pr) - Этот
# вариант правильно определяет свойство, сначала без геттера и сеттера, 
# затем добавляя сеттер и геттер поочередно. Это тоже допустимый способ 
# определения свойства с геттером и сеттером.

# pr = property(); pr = pr.setter(set_pr); pr = pr.getter(get_pr); pr = pr.deleter(del_pr) - Этот вариант 
# правильно определяет свойство с геттером, сеттером и делитером, 
# добавляя их последовательно.
